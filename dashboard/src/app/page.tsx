...
const NAV_ITEMS = [
  ...
  { id: "CloudTrail",       label: "CloudTrail",       icon: <IconCloudTrail /> },
...
type CTEvent = {
  ...
  Username: string;
  ReadOnly: string;
  Resources: { ResourceName: string; ResourceType: string }[];
};

function CloudTrailTab() {
  ...
  const load = () => {
    ...
    fetch(`/api/cloudtrail?${params}`, { cache: "no-store" })
      .then(r => r.json())
  ...
  const services = Array.from(new Set(events.map(e => e.EventSource.replace(".amazonaws.com", "")))).sort();
  const filtered = events.filter(e => {
    if (search) {
      const q = search.toLowerCase();
      return e.EventName.toLowerCase().includes(q) || e.Username.toLowerCase().includes(q) ||
        e.EventSource.toLowerCase().includes(q) || (e.Resources || []).some(r => r.ResourceName?.toLowerCase().includes(q));
    }
    return true;
  });
  ...
  const svcColor = (src: string) => {
    const colors: Record<string, string> = { s3: "#f59e0b", lambda: "#8b5cf6", sqs: "#3b82f6", sns: "#10b981", rds: "#06b6d4", eks: "#6366f1", iam: "#ef4444", secretsmanager: "#ec4899", cloudwatch: "#14b8a6", dynamodb: "#f97316" };
    const svc = src.replace(".amazonaws.com", "");
    return colors[svc] || "#6b7280";
  };
  ...
  return (
    <div style={{ padding: "24px 32px", height: "100%", overflowY: "auto" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
        <div>
          <h2 style={{ fontSize: 20, fontWeight: 700, margin: 0 }}>CloudTrail — API Audit Log</h2>
          <p style={{ fontSize: 13, color: "var(--text-muted)", margin: "4px 0 0" }}>Every AWS API call made to KumoStack, recorded in real-time</p>
        </div>
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 16 }}>
        <input
          placeholder="Search event name, user, service, resource..."
          value={search}
          onChange={e => setSearch(e.target.value)}
          style={{ flex: 1, padding: "8px 12px", fontSize: 13, background: "var(--bg-elevated)", border: "1px solid var(--border)", borderRadius: 6, color: "var(--text-primary)", outline: "none" }}
        />
        <select value={serviceFilter} onChange={e => setServiceFilter(e.target.value)}
          style={{ padding: "8px 12px", fontSize: 13, background: "var(--bg-elevated)", border: "1px solid var(--border)", borderRadius: 6, color: "var(--text-primary)" }}>
      ...
      <div style={{ fontSize: 12, color: "var(--text-muted)", marginBottom: 12 }}>
        {loading ? "Loading…" : `${filtered.length} events${search || serviceFilter ? " (filtered)" : ""}`}
      </div>

      {!loading && filtered.length === 0 ? (
        <div style={{ textAlign: "center", padding: "60px 20px", color: "var(--text-muted)" }}>
          <div style={{ fontSize: 32, marginBottom: 12 }}>📋</div>
          <div style={{ fontSize: 15, fontWeight: 600, marginBottom: 8 }}>No events recorded yet</div>
          <div style={{ fontSize: 13 }}>CloudTrail recording is enabled. API calls will appear here automatically.</div>
        </div>
      ) : (
        <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 8, overflow: "hidden" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
            <thead>
              <tr style={{ background: "var(--bg-elevated)", borderBottom: "1px solid var(--border)" }}>
                <th style={{ padding: "10px 14px", textAlign: "left", fontWeight: 600, color: "var(--text-muted)", width: 80 }}>Time</th>
                <th style={{ padding: "10px 14px", textAlign: "left", fontWeight: 600, color: "var(--text-muted)", width: 100 }}>Service</th>
                <th style={{ padding: "10px 14px", textAlign: "left", fontWeight: 600, color: "var(--text-muted)" }}>Event</th>
                <th style={{ padding: "10px 14px", textAlign: "left", fontWeight: 600, color: "var(--text-muted)", width: 100 }}>User</th>
                <th style={{ padding: "10px 14px", textAlign: "left", fontWeight: 600, color: "var(--text-muted)", width: 60 }}>R/W</th>
                <th style={{ padding: "10px 14px", textAlign: "left", fontWeight: 600, color: "var(--text-muted)" }}>Resources</th>
              </tr>
            </thead>
            <tbody>
              {filtered.slice(0, 200).map((e, i) => {
                const svc = e.EventSource.replace(".amazonaws.com", "");
                return (
                  <tr key={e.EventId} style={{ borderBottom: i < filtered.length - 1 ? "1px solid var(--border)" : "none", background: i % 2 === 0 ? "transparent" : "rgba(255,255,255,0.01)" }}>
                    <td style={{ padding: "8px 14px", color: "var(--text-muted)", fontFamily: "monospace" }}>{fmtTime(e.EventTime)}</td>
                    <td style={{ padding: "8px 14px" }}>
                      <span style={{ fontSize: 11, fontWeight: 600, padding: "2px 7px", borderRadius: 4, background: svcColor(e.EventSource) + "22", color: svcColor(e.EventSource), border: `1px solid ${svcColor(e.EventSource)}44` }}>{svc}</span>
                    </td>
                    <td style={{ padding: "8px 14px", fontWeight: 500, color: "var(--text-primary)", fontFamily: "monospace" }}>{e.EventName}</td>
                    <td style={{ padding: "8px 14px", color: "var(--text-muted)" }}>{e.Username}</td>
                    <td style={{ padding: "8px 14px" }}>
                      <span style={{ fontSize: 10, padding: "1px 5px", borderRadius: 3, background: e.ReadOnly === "true" ? "#10b98122" : "#f59e0b22", color: e.ReadOnly === "true" ? "#10b981" : "#f59e0b", border: `1px solid ${e.ReadOnly === "true" ? "#10b98144" : "#f59e0b44"}`, fontWeight: 600 }}>
                        {e.ReadOnly === "true" ? "READ" : "WRITE"}
                      </span>
                    </td>
                    <td style={{ padding: "8px 14px", color: "var(--text-muted)" }}>
                      {(e.Resources || []).map((r, ri) => <span key={ri} style={{ fontSize: 11, fontFamily: "monospace", marginRight: 6 }}>{r.ResourceName}</span>)}
                    </td>
                  </tr>
                );
              })}
  ...
  return (
    <div className="app-shell">
      ...
      <main className={`app-main${["Diagrams","Stackport","Topology","ArchGallery"].includes(activeTab) ? " app-main--fullscreen" : ""}`}>
        ...
        {activeTab === "CloudTrail"     && <CloudTrailTab />}

...
All Ministack services and their host-mapped ports.

| Port | Service | URL | Notes |
|---|---|---|---|
| **4566** | Ministack (AWS API) | `http://localhost:4566` | All AWS service endpoints |
| **3002** | Grafana | `http://localhost:3002` | admin / admin |
| **3003** | Ministack Dashboard | `http://localhost:3003` | Main UI |
...
| **6379** | Redis | `localhost:6379` | Used by Ministack internally |
...
All services share the single Ministack endpoint at port **4566**. The endpoint is the same for every service:
...
Inside the Docker network (`ministack_default`), services use these hostnames:

| Internal hostname | Service |
|---|---|
| `ministack:4566` | Ministack AWS API |

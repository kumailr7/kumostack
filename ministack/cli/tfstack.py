#!/usr/bin/env python3
"""
tfstack — Terraform CLI wrapper for MiniStack.

Generates a temporary ``ministack_providers_override.tf`` file that redirects
every AWS provider endpoint to your local MiniStack instance, executes the
...
Environment variables:
    ...
    MINISTACK_HOSTNAME         MiniStack host           (default: localhost)
    GATEWAY_PORT               MiniStack port           (default: 4566)
    USE_SSL                    Use https instead of http (default: 0)
    S3_HOSTNAME                S3 virtual-host base     (default: s3.localhost.ministack.org)
    ...
    TS_PROVIDERS_FILE          Override filename        (default: ministack_providers_override.tf)
...
TS_PROVIDERS_FILE   = _str_env("TS_PROVIDERS_FILE", "ministack_providers_override.tf")
...
S3_HOSTNAME = _str_env("S3_HOSTNAME", "s3.localhost.ministack.org")
...
def get_endpoint(service: str) -> str:
    """Return the MiniStack endpoint URL for a Terraform AWS provider service key."""
    ...
    if canonical in _SUBDOMAIN_SERVICES:
        return f"{SCHEME}://{canonical}.localhost.ministack.org:{GATEWAY_PORT}"

...
def test_rds_aurora_cluster_members_share_one_container(monkeypatch):
    ...
    monkeypatch.setattr(m, "_get_kumostack_network", lambda _client: None)
...
def test_mysql_image_for_version_maps_aurora_tracks():
    from kumostack.services.rds import _mysql_image_for_version
...
def test_rds_restore_state_respawns_docker_container(monkeypatch):
    ...
    """
    from kumostack.core.responses import get_account_id, get_region
    from kumostack.services import rds as m
    ...
    monkeypatch.setattr(m, "_get_docker", lambda: FakeDocker())
    monkeypatch.setattr(m, "_get_kumostack_network", lambda c: None)
...
def test_rds_restore_state_respawns_one_container_per_cluster(
    ...
    from kumostack.core.responses import AccountRegionScopedDict, get_account_id, get_region
    from kumostack.services import rds as m
...
def test_rds_cluster_volume_name_cannot_match_legacy_instance():
    ...
    assert "kumostack-rds-cluster-" in m._rds_cluster_docker_volume_name(
        "orders",
    )
...
def test_rds_legacy_instance_restore_preserves_arn_region(monkeypatch):
    from kumostack.core.responses import AccountScopedDict, get_region, set_request_region
...
def test_rds_docker_artifact_names_are_region_scoped():
    from kumostack.core.responses import get_region, set_request_region
    from kumostack.services import rds
...
def test_aurora_cluster_uses_one_backing_container(rds):
    ...
    with _live_cluster(rds) as (cluster_id, _wid, _rid, writer, reader, cluster):
        containers = docker.from_env().containers.list(
            all=True,
            filters={"label": ["kumostack=rds", f"cluster_id={cluster_id}"]},
        )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (True, "rdsadmin", "kumostack-rds-control"),
    ],
)
...
def test_rds_deleting_last_global_secondary_instance_preserves_headless_applier(
    ...
    from kumostack.core.responses import get_account_id, get_region, set_request_region
    from kumostack.services import rds as m
...
def test_rds_restore_respawns_persisted_headless_secondary_applier(monkeypatch):
    ...
    from kumostack.core.responses import AccountRegionScopedDict, AccountScopedDict
    from kumostack.services import rds as m
...
def test_rds_remove_headless_global_secondary_commits_detach_and_stops_applier(
    ...
    from kumostack.core.responses import get_account_id, set_request_account_id
    from kumostack.services import rds as m

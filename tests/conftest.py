...
def reset_server(tmp_path_factory, worker_id):
    ...
    calls /_kumostack/reset on startup, a slow worker's reset can fire AFTER
    ...
    if worker_id == "master":
        ...
        try:
            urllib.request.urlopen(
                urllib.request.Request(f"{ENDPOINT}/_kumostack/reset",
                                       data=b"", method="POST"),
                timeout=5,
            )
        except Exception:
    ...
    marker = root_tmp / ".kumostack_reset_done"
    lock = root_tmp / ".kumostack_reset.lock"
    try:

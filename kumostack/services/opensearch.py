...
def reset():
    ...
    if docker is not None:
        for c in docker.containers.list(
            all=True,
            filters={"label": "com.kumostack.service=opensearch"},
        ):
            try:
                c.stop(timeout=2)
                c.remove(force=True)
...
def _container_name(domain_name: str) -> str:
    return f"kumostack-opensearch-{domain_name}"


def _dashboards_container_name(domain_name: str) -> str:
    return f"kumostack-opensearch-dashboards-{domain_name}"
...
def _spawn_dataplane(domain_name: str, engine_version: str):
    ...
    """
    stub_host = f"{domain_name}.kumostack.local"
    stub_port = 9200
    ...
    labels = {
        "com.kumostack.service": "opensearch",
        "com.kumostack.domain": domain_name,
        "com.docker.compose.project": "kumostack",
    }
    env_vars = {
    ...
    if DASHBOARDS_ENABLED:
        try:
            dash_kwargs = dict(
                ...
                labels={**labels, "com.kumostack.role": "dashboards"},
                environment={
...
def _normalise_vpc_options(options):
    ...
    out.setdefault("VPCId", "vpc-kumostack")
    out.setdefault(
        "AvailabilityZones",

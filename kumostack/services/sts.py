...
from kumostack.core.responses import get_account_id, json_response, new_uuid

# Shared helpers — IAM and STS are a natural pair; STS is stateless
# and reuses IAM's XML builders and credential generators.
from kumostack.services.iam import _error, _future, _gen_secret, _gen_session_access_key, _gen_session_token, _p, _xml
...
async def handle_request(method, path, headers, body, query_params):
    ...
    if action == "GetWebIdentityToken":
        ...
        sig = _b64url(hmac.new(b"kumostack-fake-key", f"{h}.{p}".encode(), hashlib.sha256).digest())
        token = f"{h}.{p}.{sig}"

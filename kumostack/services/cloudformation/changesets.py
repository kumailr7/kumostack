...
from kumostack.core.responses import get_account_id, new_uuid, now_iso
...
def _find_change_set(cs_name, stack_name=""):
    ...
    from kumostack.services.cloudformation import _change_sets
...
def _create_change_set(params):
    from kumostack.services.cloudformation import _change_sets, _stack_events, _stacks
...
def _list_change_sets(params):
    from kumostack.services.cloudformation import _change_sets, _stacks
    stack_name = _p(params, "StackName")
    if not stack_name:
        return _error("ValidationError", "StackName is required")

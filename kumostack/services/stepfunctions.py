...
def _invoke_sqs_send_message(resource, input_data):
    ...
    try:
        from kumostack.services import sqs
    except ImportError:
...
def _invoke_sns_publish(resource, input_data):
    ...
    try:
        from kumostack.services import sns
    except ImportError:
...
def _invoke_dynamodb(op_name, input_data):
    ...
    try:
        from kumostack.services import dynamodb
    except ImportError:
...
def _invoke_ecs_run_task(resource, input_data):
    ...
    try:
        from kumostack.services import ecs
    except ImportError:
...
# Map aws-sdk service names to KumoStack internal routing info.
...
def _dispatch_aws_sdk_json(service_info, service_name, action, input_data):
    """Dispatch an aws-sdk integration call to a JSON-protocol KumoStack service."""
    ...
    if not handler:
        raise _ExecutionError(
            "States.Runtime",
            f"Service '{service_key}' is not available in KumoStack",
        )
...
def _flatten_ec2_query_params(data, prefix=""):
    ...
    Most query services in KumoStack use ``member.N`` in the Step Functions
...
def _dispatch_aws_sdk_query(service_info, service_name, action, input_data):
    """Dispatch an aws-sdk integration call to a query-protocol KumoStack service."""
...
def _dispatch_aws_sdk_rest_json(service_info, service_name, action, input_data):
    """Dispatch an aws-sdk integration call to a REST-JSON protocol KumoStack service."""
    from kumostack import app
...
def _dispatch_aws_sdk_rest_xml(service_info, service_name, action, input_data):
    ...
    if not spec:
        raise _ExecutionError(
            "States.Runtime",
            f"aws-sdk:{service_name}:{action} is not yet implemented in KumoStack "
...
def _invoke_aws_sdk_integration(resource, input_data):
    """Dispatch arn:aws:states:::aws-sdk:<service>:<action> to the target KumoStack service."""
    ...
    if not service_info:
        raise _ExecutionError(
            "States.Runtime",
            f"Service '{service_name}' is not supported in KumoStack aws-sdk integrations",
        )

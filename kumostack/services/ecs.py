...
# ECS exited-container reaper. Every kumostack=ecs container we start via
...
def _reap_exited_containers():
    ...
    for c in docker_client.containers.list(all=True, filters={"label": "kumostack=ecs"}):
...
def _ensure_ecs_reaper_thread():
    ...
    with _ecs_reaper_lock:
        ...
        threading.Thread(target=_loop, daemon=True, name="kumostack-ecs-reaper").start()
...
def _register_metadata(task_arn, cluster_arn, td, cdef, launch_type, env,
    ...
    `kumostack_net_ip` is KumoStack's IP on the shared user-defined network
    ...
    if host_mode:
        host = "127.0.0.1"
    elif kumostack_net_ip:
        host = kumostack_net_ip
    else:
...
def _build_run_kwargs(cdef, td, env, port_bindings, ecs_network,
    ...
    kwargs = dict(
        ...
        name=f"kumostack-ecs-{task_id[:8]}-{cdef['name']}",
        labels={"kumostack": "ecs", "task_arn": task_arn},
    ...
    if host_mode:
        ...
        if not kumostack_net_ip:
            # Docker-Desktop fallback: when we couldn't discover KumoStack's
...
def _run_task(data):
    ...
    for _ in range(count):
        ...
        if docker_client and td:
            ...
            # Detect the Docker network KumoStack is running on,
            # so ECS containers can reach sibling services (S3, etc.)
            ecs_network = None
            kumostack_net_ip = None
            try:
                ...
                if nets:
                    ...
                    # IP that ECS containers should use to reach KumoStack.
                    # Using kumostack's own address on the shared network is
                    ...
                    kumostack_net_ip = nets[ecs_network].get("IPAddress") or None
                    logger.debug(
                        "ECS: detected KumoStack network=%s ip=%s",
                        ecs_network, kumostack_net_ip,
                    )
            except Exception:
...
# ---------------------------------------------------------------------------
# Container instances (stub — KumoStack runs tasks as Docker containers
...
def reset():
    docker_client = _get_docker()
    if docker_client:
        # Stop + remove every kumostack=ecs container (running OR exited).

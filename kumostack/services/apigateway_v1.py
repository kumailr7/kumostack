...
from kumostack.services.apigateway import _timeout_from_env, _urlopen_async
...
def _encode_rest_api_policy(policy):
    ...
    policy string (what kumostack used to do) makes the provider's decoder
...
async def _invoke_lambda_proxy_v1(integration, api_id, stage_name, stage, resource, request_path, method, headers, body, query_params, path_params):
    ...
    from kumostack.services import lambda_svc as _lambda_svc
    if "function:" in uri:
...
def _resolve_custom_rest_api_id(tags: dict) -> tuple[str | None, tuple | None]:
    ...
    Reads the kumostack-native ``ms-custom-id`` tag (issue #400). If the
    LocalStack ``ls-custom-id`` tag is set (and ``ms-custom-id`` is not), the
    caller gets a clear ``BadRequestException`` so the kumostack-native key is

...
DATA_DIR = os.environ.get("S3_DATA_DIR", "/tmp/kumostack-data/s3")
...
def _list_buckets():
    ...
    SubElement(owner, "DisplayName").text = "kumostack"
...
def _get_bucket_acl(name: str):
    ...
    body = (
        ...
        b"<Owner><ID>owner-id</ID><DisplayName>kumostack</DisplayName></Owner>"
        b"<AccessControlList><Grant>"
        b'<Grantee xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:type="CanonicalUser">'
        b"<ID>owner-id</ID><DisplayName>kumostack</DisplayName></Grantee>"
...
def _enforce_post_policy_size(policy_b64: str, size: int):
    ...
    same lenient stance as kumostack's presigned-URL handling.
    """
    if not policy_b64:
        return None
    try:
...
def _post_object(bucket_name: str, body: bytes, headers: dict):
    ...
    ignored — same lenient stance as kumostack's presigned-URL handling.
    """
    bucket = _ensure_bucket(bucket_name)
    if bucket is None:
        return _no_such_bucket(bucket_name)
...
def _list_multipart_uploads(bucket_name: str, query_params: dict):
    ...
    for uid, upload in uploads[:max_uploads]:
        ...
        SubElement(initiator, "DisplayName").text = "kumostack"
...
def reset():
    """Wipe all in-memory state (used by /_kumostack/reset)."""

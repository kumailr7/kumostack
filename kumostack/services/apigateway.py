...
from kumostack.core.responses import AccountScopedDict, error_response_json, get_account_id, get_region, new_uuid
...
# JWKS cache is account-scoped because issuer URLs in KumoStack can resolve
...
async def handle_request(method, path, headers, body, query_params):
    ...
    if parts and parts[0] in ("restapis", "apikeys", "usageplans", "domainnames", "tags", "account"):
        from kumostack.services import apigateway_v1
...
def _resolve_custom_api_id(tags: dict, existing: "AccountScopedDict") -> str | None:
    ...
    ``ms-custom-id`` so the kumostack-native key is the only contract."""
...
async def handle_websocket(scope, receive, send, api_id: str, path_override: str | None = None):
    ...
    try:
        ...
        conn_record = {
            ...
            "stage": stage,
            # Int epoch seconds — matches kumostack JSON timestamp convention.

...
from kumostack.core.arn import Arn, ArnParseError, parse_arn
...
def _collect_sqs():
    import kumostack.services.sqs as svc
...
def _collect_eventbridge():
    import kumostack.services.eventbridge as svc
    for arn, tags in svc._tags.items():
        yield arn, _normalise_flat(tags)


def _collect_kms():
    import kumostack.services.kms as svc
...
def _collect_ecr():
    import kumostack.services.ecr as svc
...
def _collect_ecs():
    import kumostack.services.ecs as svc
    for arn, tags in svc._tags.items():
        yield arn, _normalise_ecs(tags)


def _collect_glue():
    import kumostack.services.glue as svc
    for arn, tags in svc._tags.items():
        yield arn, _normalise_flat(tags)


def _collect_cognito():
    import kumostack.services.cognito as svc
...
def _collect_appsync():
    import kumostack.services.appsync as svc
    for arn, tags in svc._tags.items():
        try:
            spec = parse_arn(arn)
...
def _collect_scheduler():
    import kumostack.services.scheduler as svc
    for arn, tags in svc._tags.items():
        yield arn, _normalise_flat(tags)


def _collect_cloudfront():
    import kumostack.services.cloudfront as svc
    for arn, tags in svc._tags.items():
        yield arn, _normalise_list(tags)


def _collect_efs():
    import kumostack.services.efs as svc
...
def _collect_backup():
    import kumostack.services.backup as svc
...
def _collect_elasticache():
    import kumostack.services.elasticache as svc
    for arn, tags in svc._tags.items():
        yield arn, _normalise_list(tags)

...
# asyncssh is an optional extra (pip install kumostack[full]). We import
...
async def _create_server(data):
    ...
    # endpoint /_kumostack/transfer/sftp-ports.
    try:
...
def _describe_server(data):
    ...
    # SFTP port discovery is via GET /_kumostack/transfer/sftp-ports —
...
def _sftp_host_key_path() -> str:
    from kumostack.core.persistence import STATE_DIR
...
def _sftp_load_or_generate_host_key():
    ...
    from kumostack.core.persistence import PERSIST_STATE
...
def _sftp_s3_bucket(account_id: str, bucket: str):
    ...
    """
    from kumostack.services import s3 as s3_mod
...
def _sftp_build_sftp_factory(restrict_to_server_id: Optional[str] = None):
    ...
    def _factory(chan):
        return _KumoStackSFTPServer(chan, restrict_to_server_id=restrict_to_server_id)
    return _factory
...
def _sftp_build_server_factory(restrict_to_server_id: Optional[str] = None):
    ...
    def _factory():
        return _KumoStackSSHServer(restrict_to_server_id=restrict_to_server_id)
    return _factory


class _KumoStackSSHServer(asyncssh.SSHServer if _ASYNCSSH_AVAILABLE else object):
    ...
    def validate_public_key(self, username, key):
        ...
        if self._conn is not None:
            self._conn.set_extra_info(kumostack_user=user)
            self._conn.set_extra_info(kumostack_server_id=server_id)
            self._conn.set_extra_info(kumostack_account_id=account_id)
        return True


class _KumoStackSFTPServer(asyncssh.SFTPServer if _ASYNCSSH_AVAILABLE else object):
    """SFTP filesystem backed by KumoStack's in-memory S3 state.

    asyncssh instantiates this per session, passing the SSH channel so we
    can recover the auth metadata stashed by `_KumoStackSSHServer`.
    ...
    def __init__(self, chan, restrict_to_server_id: Optional[str] = None):
        ...
        if hasattr(chan, "get_connection"):
            ...
            conn = chan._conn
        self._user = conn.get_extra_info("kumostack_user") if conn else None
        self._account_id = conn.get_extra_info("kumostack_account_id") if conn else None
        self._server_id = conn.get_extra_info("kumostack_server_id") if conn else None
    ...
    def _format_longname(self, name, attrs) -> str:
        ...
        return f"{mode_str} 1 kumostack kumostack {size:>10d} {ts} {name}"
...
def sftp_per_server_port(server_id: str) -> Optional[int]:
    ...
    Used by the `/_kumostack/transfer/sftp-ports` admin endpoint."""
    return _sftp_per_server_ports.get(server_id)

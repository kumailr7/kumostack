...
def _get_kumostack_network(client):
...
def _k3s_run_kwargs(name: str, port: int, ms_network: str | None = None) -> dict:
    ...
    run_kwargs = dict(
        ...
        devices=["/dev/fuse"],
        ports={"6443/tcp": port},
        name=f"kumostack-eks-{name}",
        labels={"kumostack": "eks", "cluster_name": name},
...
def _stop_all_k3s():
    """Stop all k3s containers managed by KumoStack."""
    client = _get_docker()
    if not client:
        return
    try:
        for c in client.containers.list(filters={"label": "kumostack=eks"}):
            try:
                c.stop(timeout=5)

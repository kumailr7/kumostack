...
_STATE_DIR = os.environ.get("STATE_DIR", "/tmp/kumostack-state")
...
def well_known_openid_configuration(pool_id: str, region: str | None = None, host: str | None = None):
    ...
    KumoStack gateway where /oauth2/authorize, /oauth2/token, /oauth2/userInfo
    and /logout are actually served (added by PR #344). Real AWS serves these
    on the pool-domain host; KumoStack serves them on the gateway.
    """
...
def _fake_token(sub: str, pool_id: str, client_id: str, token_type: str = "access",
    ...
    header = base64.urlsafe_b64encode(
        json.dumps({"alg": "RS256", "kid": "kumostack-key-1"}).encode()
    ).rstrip(b"=").decode()
    now = int(time.time())
    origin_jti = new_uuid()
    claims = {
...
def _deliver_cognito_email(pool, to_email, subject, body, type_name, extra=None):
    ...
    try:
        from kumostack.services import ses as ses_mod  # local import: avoid cycle
...
<div class="footer">Powered by kumostack</div>
</div>
</body>
</html>"""
...
def _decode_id_token_unverified(id_token: str) -> dict:
    ...
    Matches KumoStack's wider stance on emulator-side crypto checks: we don't
...
def _oauth2_idp_response(method, body, query_params):
    ...
    the IdP redirects back to KumoStack with `code` and `state` in the query
    string (or POST body, depending on the IdP's `response_mode`). KumoStack
    ...
    user, and redirects to the app's `redirect_uri` with a KumoStack-issued
    ...
    # Issue KumoStack auth code so the app's /oauth2/token call works.
    _cleanup_expired_relay_codes()

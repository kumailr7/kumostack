...
# scheduled, kumostack schedules a re-invocation at the earliest WAIT expiry.
import heapq
...
def _fire_chained_invoke(parent_rec: dict, op_id: str, ci_opts: dict,
    ...
    The child runs via the existing lambda_svc executor; kumostack records
    ...
    def _run():
        try:
            ...
            # but in a worker thread so we don't block kumostack's ASGI loop.
...
# ---------------------------------------------------------------------------
# Reset hook for kumostack's /_kumostack/reset.
# ---------------------------------------------------------------------------

...
from kumostack.core.lambda_runtime import get_or_create_worker, invalidate_worker
from kumostack.core.persistence import PERSIST_STATE, STATE_DIR, load_state
...
# container instead of running in kumostack's worker pool. The container
...
_PROVIDED_CODE_CACHE = os.path.join(tempfile.gettempdir(), "kumostack-provided-code")
...
def _rewrite_host_for_container(url: str) -> str:
    ...
    Docker Lambda container can reach kumostack on the host. Explicitly
...
def _get_function(name: str, qualifier: str | None = None):
    ...
    if effective_config.get("PackageType") == "Image" and effective_config.get("ImageUri"):
        ...
        # a kumostack-internal endpoint dressed up with the AWS query params
...
def _presigned_code_url(func_name: str) -> str:
    ...
    qs = (
        ...
        f"&X-Amz-Signature=kumostack-local-presigned"
    )
    return (
        f"http://{host}:{port}/_kumostack/lambda-code/"
...
def _ensure_reaper_thread() -> None:
    ...
    with _reaper_lock:
        ...
        threading.Thread(target=_loop, daemon=True, name="kumostack-lambda-reaper").start()
...
def _invoke_rie(container, event: dict, timeout: int) -> dict:
    ...
    import urllib.request
    # A CloudFormation custom-resource ResponseURL points at kumostack on the
...
def _spawn_lambda_container(config: dict, code_zip: bytes | None):
    ...
    if package_type == "Zip":
        ...
        tmpdir = tempfile.mkdtemp(prefix="kumostack-lambda-docker-")
...
def _emit_lambda_logs(func: dict, request_id: str, log_text: str,
    ...
    """
    try:
        from kumostack.services import cloudwatch_logs as _cwl
        config = func.get("config") or func
        fn_name = config.get("FunctionName", "unknown")
...
def _layer_content_url(layer_name: str, version: int) -> str:
    ...
    return (
        f"http://{_MINISTACK_HOST}:{port}/_kumostack/lambda-layers/"
...
def reset():
    ...
    from kumostack.core import lambda_runtime

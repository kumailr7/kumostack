...
// that routes through AWS_ENDPOINT_URL (KumoStack).
(function _installAwsSdkV3Stubs() {
  ...
  // KumoStack's router maps target prefixes to service modules.
  const _JSON_RPC_TARGETS = {
    // JSON-RPC (awsJson1.x) services — keyed by @aws-sdk/client-{key} suffix.
    // Target prefixes match KumoStack's router.py SERVICE_PATTERNS target_prefixes.
    "ssm":                         "AmazonSSM",
    ...
    // All three are handled by KumoStack's native query/path routing when the real SDK is present
...
function patchAwsSdk() {
  ...
  https.request = function(options, callback) {
    ...
    // so the PUT reaches KumoStack's HTTP server on msPort, not port 443.
...
class Worker:
    ...
    def _spawn(self):
        ...
        if layer_refs:
            from kumostack.services.lambda_svc import _resolve_layer_zip
        ...
        # default that points at this KumoStack instance.  Real AWS Lambda

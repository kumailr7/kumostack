...
def resolve_tls_material() -> "tuple[str, str]":
    ...
    otherwise auto-generate a self-signed cert under `${TMPDIR}/kumostack-tls/`
    ...
    tls_dir = os.path.join(tempfile.gettempdir(), "kumostack-tls")
    ...
    if not (os.path.exists(cert_path) and os.path.exists(key_path)):
        subprocess.run([
            ...
            "-days", "825",
            "-subj", "/CN=kumostack-local/O=KumoStack",
            "-addext", "subjectAltName=DNS:localhost,DNS:kumostack,IP:127.0.0.1,IP:0:0:0:0:0:0:0:1",
